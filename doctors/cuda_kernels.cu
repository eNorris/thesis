        ...
        int startIndx, int voxThisSubSweep, int *gpuIdxToMesh)
{
    ...
    //if(ix == 32 && iy == 32 && iz == 8)
    //{
    //printf("ix=%d, iy=%d, iz=%d, psi=%e, totSrc=%e, inScat=%e, cFlux=%e, numer=%e, inx=%e, iny=%e, inz=%e, sig=%e, zid=%d, ie=%d, groups=%d, pn=%d, indx=%d\n",
    //       ix, iy, iz,
    //       angFlux, totalSource[ir], inscatter, colFlux[ie*Nx*Ny*Nz + ir], numer, influxX, influxY, influxZ,
    //       scatxs2d[zid*groups*groups*pn + ie*groups*pn +  ie*groups], zid, ie, groups, pn, zid*groups*groups*pn + ie*groups*pn +  ie*pn);
    //}
    ...
    //{
    //    printf("after tempSummer(ia=%d) angFlux=%e, wt=%e, tempFlux=%e\n", iang, angFlux, wt[iang], tempFlux[ir]);
    //}
}
        ...
        int Nx, int Ny, int Nz, int ie)
{
    ...
    //if(ir == 32*64*16 + 32*16 + 8)
    //{
    //    printf("@clearSweek, cFlux=%e, tempFlux=%e BEFORE clear\n", cFlux[ie*Nx*Ny*Nz + ir], tempFlux[ir]);
    //}
    ...
    //if(ir == 32*64*16 + 32*16 + 8)
    //{
    //    printf("@clearSweek, cFlux=%e, tempFlux=%e AFTER clear\n", cFlux[ie*Nx*Ny*Nz + ir], tempFlux[ir]);
    //}
}
